'use client'
import { FolderTree, CalendarRange, Users, Clipboard, MenuSquare, LayoutDashboard, Menu, ShoppingBag, ClipboardList, X, Store, TableProperties, Warehouse, MoreVertical, Settings, Moon, Sun } from "lucide-react";
...
export default function Navigation({payload}) {
	const { user, autenticado, verificarToken, setUser, setAutenticado } = useAuth()
							...
							settings.tipo === "RESTAURANTE" ?
								...
								<ul className="flex flex-col gap-2">
									...
									<Dropdown titulo="Catálogos" icon={<FolderTree />} items={[
										{ text: "Clientes", goTo: goTo, icon: <ClipboardList />, linkto: "/catalogos/clientes" },
										{ text: "Empleados", goTo: goTo, icon: <ClipboardList />, linkto: "/catalogos/empleados" },
										{ text: "Productores", goTo: goTo, icon: <ClipboardList />, linkto: "/catalogos/productors" },
										{ text: "Productos", goTo: goTo, icon: <ClipboardList />, linkto: "/catalogos/productos" },
										{ text: "Ubicaciones", goTo: goTo, icon: <ClipboardList />, linkto: "/catalogos/ubicaciones" },
									]}
									/>
